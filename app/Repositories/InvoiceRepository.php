    ...
    try {
        ...
        if (isset($input['file'])) {
            ...
            foreach ($input['file'] as $file) {
                $imageName = time() . '_' . $file->getClientOriginalName(); // Generate unique filename
                $file->move(public_path('images'), $imageName); // Move file to the 'public/images' directory
                $imagePaths[] = 'images/' . $imageName; // Save the relative path to the array
            }
        ...
        $input = Arr::only($input, [
            ...
            'note', 'term', 'template_id', 'payment_qr_code_id', 'status', 'file', 'tax_id', 'tax', 'currency_id', 'recurring_status', 'recurring_cycle',
        ]);
