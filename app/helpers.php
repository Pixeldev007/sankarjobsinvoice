    ...
    function getInvoicePaidAmount($invoiceId)
    {
        ...
        if ($invoice->status != Invoice::PAID) {
            foreach ($invoice->payments as $payment) {
                // Exclude manual payments from calculation
...
if (! function_exists('getInvoiceCurrencyAmount')) {
    /**
     * Get the formatted invoice amount.
     *
     * @param float|int $amount The amount to format.
     * @param int|string|null $currencyId The currency ID.
     * @param bool $formatting Whether to apply formatting to the amount.
     * @param bool $withSymbol Whether to include the currency symbol.
     * @return string The formatted amount.
     */
    function getInvoiceCurrencyAmount($amount, $currencyId = null, $formatting = false, $withSymbol = true): string
    {
        static $currencyPosition;
        static $currencySymbols;

        // Get the currency position if not set
        if (empty($currencyPosition)) {
            $currencyPosition = getSettingValue('currency_after_amount');
        }

        // Get the current currency ID if not provided
        if (empty($currencyId)) {
            $currencyId = getSettingValue('current_currency');
        }

        // Get the currency symbols if not set
        if (empty($currencySymbols)) {
            $currencySymbols = Currency::toBase()->pluck('icon', 'id')->toArray();
        }

        // Format the amount based on the formatting parameter
        $formattedAmount = $formatting ? numberFormat($amount) : formatTotalAmount($amount);

        // Get the currency symbol if symbols are available
        $currencySymbol = isset($currencySymbols[$currencyId]) ? $currencySymbols[$currencyId] : '₹';

        // Check if the symbol should be included
        if (!$withSymbol) {
            return $formattedAmount;
        }

        // Return the formatted amount with or without the currency symbol
        if ($currencyPosition) {
            return $formattedAmount . ' ' . $currencySymbol;
        }
