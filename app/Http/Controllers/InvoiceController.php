...
class InvoiceController extends AppBaseController
{
    ...
    public function exportInvoicesPdf(): Response
    {
        ...
        $pdf = Pdf::loadView('invoices.export_invoices_pdf', $data);
         ...
         return $pdf->stream('Invoices.pdf'); // View the PDF in the browser
    ...
    public function imgEdit($invoiceId)
{
    // Fetch the invoice from the database using the provided ID
    $invoice = Invoice::findOrFail($invoiceId);
    
    // Pass the invoice variable to the view
    return view('invoices.edit-img', compact('invoice'));
}

    public function updateImages(Request $request, $invoiceId)
    {
        ...
        if (!empty($input['file'])) {
            ...
            $input['file'] = implode(',', $imagePaths);

            // Save image paths to the invoice
            $invoice->file = $input['file'];
            $invoice->save();
        }

        // Redirect back with a success message
        return redirect()->route('invoices.index')->with('success', 'Images updated successfully.');
    }
    public function exportPdf(Request $request)
{
    ...
    $invoicesQuery = Invoice::with(['client.user.media', 'payments']);

    if ($status && $status != \App\Models\Invoice::STATUS_ALL) {
        $invoicesQuery->where('invoices.status', $status);
    }
    ...
    if (!empty($dateRange)) {
        ...
        if (count($dates) == 2) {
            ...
            $invoicesQuery->whereBetween('invoices.invoice_date', [$startDate, $endDate]);
        }
    }
    ...
    // Generate PDF
    $pdf = Pdf::loadView('invoices.export_invoices_pdf', compact('invoices'));
 ...
 public function exportExcel(Request $request)
{
    ...
    $invoices = $invoicesQuery->get();
    // Export to Excel
    return Excel::download(new AdminInvoicesExport($invoices), 'AdminInvoicesExport.xlsx');
}
